...
function renderProducts(products, container, type = 'grid') {
  ...
  if (products.length === 0) {
    container.innerHTML = '<p class="empty-message">Không tìm thấy sản phẩm nào.</p>';
    return;
  }
  ...
  products.forEach(product => {
    ...
    const badges = [];
    if (product.isNew) badges.push('<span class="product-badge badge-new">Mới</span>');
    if (product.isSale) badges.push('<span class="product-badge badge-sale">Giảm giá</span>');
    if (product.isHot) badges.push('<span class="product-badge badge-hot">Hot</span>');
...
function renderCartItems() {
  ...
  document.querySelectorAll('.product-quantity input').forEach(input => {
    input.addEventListener('change', () => {
      const productId = parseInt(input.closest('.product-quantity').querySelector('.quantity-btn').dataset.id);
      let quantity = parseInt(input.value);
  ...
  wishlist.forEach(item => {
    ...
    wishlistItem.innerHTML = `
      ...
      <div class="product-actions">
        <button class="btn add-to-cart" data-id="${item.id}">Thêm vào giỏ</button>
      </div>
    `;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (document.querySelector('.category-section')) {
    ...
    if (type) {
      ...
      document.getElementById('categoryTitle').textContent = getCategoryName(type);
      document.getElementById('categoryDescription').textContent = getCategoryDescription(type);
...
function getCategoryBanner(type) {
  ...
  return banners[type] || 'https://via.placeholder.com/1200x400?text=Gundam+Collection';
}
