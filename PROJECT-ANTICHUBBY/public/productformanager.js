...
document.querySelectorAll(".edit-product").forEach((btn) => {
  btn.addEventListener("click", () => {
    ...
    const newName = prompt("Nhập tên mới cho sản phẩm:", title.textContent);
    if (!newName) return;

    const newPriceStr = prompt(
      "Nhập giá mới (VD: 1500000):",
      priceTag.textContent.replace(/\D/g, "")
    );
    ...
    priceTag.textContent = newPrice.toLocaleString("vi-VN") + "đ";

    // Đồng bộ thuộc tính data
    card.dataset.name = newName;
    card.dataset.price = newPrice;
    const addBtn = card.querySelector(".add-to-cart");
    addBtn.dataset.name = newName;
